/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { OrderUpdateManyWithoutJobsitesInput } from "./OrderUpdateManyWithoutJobsitesInput";
import { Type } from "class-transformer";
import { ProductOrderUpdateManyWithoutJobsitesInput } from "./ProductOrderUpdateManyWithoutJobsitesInput";
import { ProductStockUpdateManyWithoutJobsitesInput } from "./ProductStockUpdateManyWithoutJobsitesInput";
@InputType()
class JobsiteUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => OrderUpdateManyWithoutJobsitesInput,
  })
  @ValidateNested()
  @Type(() => OrderUpdateManyWithoutJobsitesInput)
  @IsOptional()
  @Field(() => OrderUpdateManyWithoutJobsitesInput, {
    nullable: true,
  })
  orders?: OrderUpdateManyWithoutJobsitesInput;

  @ApiProperty({
    required: false,
    type: () => ProductOrderUpdateManyWithoutJobsitesInput,
  })
  @ValidateNested()
  @Type(() => ProductOrderUpdateManyWithoutJobsitesInput)
  @IsOptional()
  @Field(() => ProductOrderUpdateManyWithoutJobsitesInput, {
    nullable: true,
  })
  productOrders?: ProductOrderUpdateManyWithoutJobsitesInput;

  @ApiProperty({
    required: false,
    type: () => ProductStockUpdateManyWithoutJobsitesInput,
  })
  @ValidateNested()
  @Type(() => ProductStockUpdateManyWithoutJobsitesInput)
  @IsOptional()
  @Field(() => ProductStockUpdateManyWithoutJobsitesInput, {
    nullable: true,
  })
  productStocks?: ProductStockUpdateManyWithoutJobsitesInput;
}
export { JobsiteUpdateInput };
