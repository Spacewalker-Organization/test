/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductStockWhereInput } from "./ProductStockWhereInput";
import { Type } from "class-transformer";
import { ProductStockOrderByInput } from "./ProductStockOrderByInput";

@ArgsType()
class ProductStockFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ProductStockWhereInput,
  })
  @Field(() => ProductStockWhereInput, { nullable: true })
  @Type(() => ProductStockWhereInput)
  where?: ProductStockWhereInput;

  @ApiProperty({
    required: false,
    type: [ProductStockOrderByInput],
  })
  @Field(() => [ProductStockOrderByInput], { nullable: true })
  @Type(() => ProductStockOrderByInput)
  orderBy?: Array<ProductStockOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ProductStockFindManyArgs };
